...
public class COMORGA_MP2 {
	...
	public static void main(String[] args) throws Exception {
		...
		// In the future, strictly return int flags and remove all return <String> or null statements
		...
		while(!(str = br.readLine()).equalsIgnoreCase("exit")) {
			...
			switch(tokens[0]) {
				case "MOV":
					MovPrint.display(o, m);
					break;
				case "ADD":
					AddPrint.display(o, m);
					break;
				case "INC":
					System.out.println("I am executed!");
					InsPrint.display(o, m);
					break;
			}
	...
	private static String parseComma(int numArgs, String str) { // fix for single or no parameter instructions
		if(numArgs < 2 || str.replaceAll(","," ").split("[ ]+").length <= 2)
			return str;
	...
	private static int chkIns(HashMap <String, String> syntax, String[] tokens) {
		...
		seq = seq.replaceAll("_\\d+", "");
		o.setOpSequence(seq);
		
		String[] opContent = Arrays.copyOfRange(tokens, 1, tokens.length); // use linked list soon for O(1)
	...
	private static String[] chkMemory(String val) {
		...
		String operators = val.replaceAll("[A-Za-z0-9_]", "");
		...
		for(int i = 0; i < tokens.length; i++) { // identify first what kind of token they are
			...
			if(i < tokens.length - 1)
				scale += Character.toString(operators.charAt(i));
		}
		...
		if(Arrays.binarySearch(scaleFactor, scale) >= 0) {
			m.setMemSequence(scale);
			m.setMemContent(tokens);
			return tokens; // fix this soon to remove redundancy with Memory object
		}
		return null;
	}
